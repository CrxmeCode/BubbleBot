        ...
        if (numb > 1 && numb <= 100) {
            ...
            try {
                ...
                Message msg = event.getTextChannel().sendMessage(
                        new EmbedBuilder().setColor(Color.GREEN).setDescription("Es wurden " + args[0] + " Nachrichten gelöscht!").build()
                ).complete();
            ...
            event.getTextChannel().sendMessage(
                    error.setDescription("Bitte gebe eine eine Zahl zwischen 2 und 100 an.").build()
            ).queue();
